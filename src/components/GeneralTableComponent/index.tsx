...
import { Props, BaseData } from "./types";
import {
  Container,
  Button,
  Table,
...
export const GeneralTableComponent: <T extends BaseData>(
      ...
      <Table aria-label="simple table" className="DataTable">
        ...
        <TableBody>
          {props.data.map(item => {
            ...
            props.tableData.forEach(key => {
              cells.push(
                <TableCell align="left">
                  {key.inContent
                    ? item.content.en[key.key as string]
                    : item[key.key]}
                </TableCell>
              );
            });
            ...
            return <TableRow key={item.key}>{cells}</TableRow>;
          })}
        </TableBody>
      </Table>
      <Button
